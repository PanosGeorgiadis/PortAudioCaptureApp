// another attempt for a simple command line parser
...
namespace cmd_line_parser
{
    ...
    struct Parser
    {
        ...
        {
            bool ret = m_cmds
                .emplace(name, Argument{ shorthand, is_boolean ? "false" : "", descr, is_required,
                                   is_boolean })
                .second;
            if (ret)
            {
    ...
    private:
        ...
        char** m_argv;
        int m_required;
        std::unordered_map<std::string, Argument> m_cmds;
